    ...
    function genTabs(){
        ...
        return(
            <TabContainer>
                <TabColumn>
                    {col1.map((tab,i) => {
                        return(<Tab onUpdate={props.onUpdate} nostar={props.nostar} nodelete={props.nodelete} key={"col1" + i} data={tab}/>)
                    })}
                </TabColumn>

                <TabColumn>
                    {col2.map((tab,i) => {
                        return(<Tab onUpdate={props.onUpdate} nostar={props.nostar} nodelete={props.nodelete} key={"col2" + i} data={tab}/>)
                    })}
                </TabColumn>

                <TabColumn>
                    {col3.map((tab,i) => {
                        return(<Tab onUpdate={props.onUpdate} nostar={props.nostar} nodelete={props.nodelete} key={"col3" + i} data={tab}/>)
                    })}
                </TabColumn>
